...
- **Flat root supported**: the frozen runtime addendum explicitly supports `capabilities: ["."]`; root `oats.json` and the three canonical root skills are final.
- `TODO(engine-consumer-fixtures)`: run the released OATS 0.19.0 acquire → lock → activate → spawn probe when WS1 fixtures are available. This package has no executable surface and should not require `oats trust`.
